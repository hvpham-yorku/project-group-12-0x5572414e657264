...
def simulate_customer(age, sex, path_points, products):
    ...
    customer = add_customer(
        Customer(entered_at=enter_time, store_id=store.store_id, age=age, sex=sex)
    )
    ...
    for p in products:
        add_purchase(
            Purchase(
                product_id=p.product_id, checkout_id=checkout.checkout_id, quantity=1
            )
        )
