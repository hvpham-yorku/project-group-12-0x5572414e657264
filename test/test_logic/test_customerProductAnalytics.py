...
class TestCustomerProductAnalytics(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Remove any cached version first so the latest code is loaded.
        """
        if "src.logic.customerProductAnalytics" in sys.modules:
            del sys.modules["src.logic.customerProductAnalytics"]

        cls.cpa = importlib.import_module("src.logic.customerProductAnalytics")

    def setUp(self):
        """
        Patch all table select() calls before every test so the tests
        use fake data instead of the real database.
        """
        self.patcher_product_select = patch.object(self.cpa.ProductTable, "select")
        self.mock_product_select = self.patcher_product_select.start()
        self.addCleanup(self.patcher_product_select.stop)

        self.patcher_purchase_select = patch.object(self.cpa.PurchaseTable, "select")
        self.mock_purchase_select = self.patcher_purchase_select.start()
        self.addCleanup(self.patcher_purchase_select.stop)

        self.patcher_checkout_select = patch.object(self.cpa.CheckoutTable, "select")
        self.mock_checkout_select = self.patcher_checkout_select.start()
        self.addCleanup(self.patcher_checkout_select.stop)

        self.patcher_customer_select = patch.object(self.cpa.CustomerTable, "select")
    ...
    def test_get_product_most_common_gender(self):
        """
        Test that the function returns the most common sex for each product.
        """
        ...
        result = self.cpa.get_product_most_common_gender()
    ...
    def test_get_product_most_common_gender_skips_empty_gender(self):
        """
        Test that customers with empty sex values are ignored.
        """
        self.mock_product_select.return_value = [
            SimpleNamespace(product_id=1, name="Milk"),
        ]
    ...
    def test_get_product_most_common_gender_returns_empty_dict_when_no_purchases(self):
        """
        Test that no purchases returns an empty dictionary.
        """
        self.mock_product_select.return_value = []
        self.mock_checkout_select.return_value = []
        self.mock_customer_select.return_value = []
    ...
    def test_get_product_most_common_gender_skips_missing_checkout(self):
        """
        Test that a purchase is ignored if its checkout does not exist.
        """
        self.mock_product_select.return_value = [
            SimpleNamespace(product_id=1, name="Milk"),
        ]
    ...
    def test_get_product_most_common_age(self):
        """
        Test that the function returns the most common age group for each product.
        """
        self.mock_product_select.return_value = [
            SimpleNamespace(product_id=1, name="Milk"),
            SimpleNamespace(product_id=2, name="Bread"),
        ...
        result = self.cpa.get_product_most_common_age()
